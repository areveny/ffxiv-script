...
class SqliteIngestionDB(IngestionDB):
    ...
    def upload_lines(self, db_values):
        ...
        with con:
            con.executemany(
                "INSERT OR REPLACE INTO lines (quest_id, text_id, speaker, text) VALUES (?, ?, ?, ?)",
                db_values,
            )
        con.close()
    ...
    def upload_quests(self, quest_values):
        ...
        with con:
            con.executemany(
                "INSERT OR REPLACE INTO quests (quest_id, quest_name, level, place_id) VALUES (?, ?, ?, ?)",
                quest_values,
            )
        con.close()
    ...
    def upload_places(self, place_values):
        ...
        with con:
            con.executemany(
                "INSERT OR REPLACE INTO places (place_id, place_name) VALUES (?, ?)",
                place_values,
            )
        con.close()
...
if __name__ == "__main__":
    ...
    lines_by_quest_id = (
        'select * from lines WHERE quest_id="clshrv530_02049" ORDER BY rowid'
    )
