...
class Ingestion():
    ...
    def process_cutscenes(self, cutscene_glob):
        cutscene_files = self.get_files_from_glob(cutscene_glob)[:2]
        processed = 0
        for cutscene_file in cutscene_files:
            self.process_cutscene_lines_file(cutscene_file)
            processed = processed + 1
            print('Ingested ({}/{}) cutscene CSVs into DB.'.format(processed, len(cutscene_files)), end='\r', flush=True)
        print('\nDone processing cutscene texts.')

    def process_cutscene_lines_file(self, cutscene_file):
        print(cutscene_file)
        path, dir, filanme = cutscene_file
        with open(path, 'r') as f:
            reader = csv.reader(f, delimiter=',', quotechar='"')
            cur_row = next(reader)
            while cur_row[0] != '0':
            ...
            for row in reader:
                ...
                if len(split_label) == 5:
                    text_type, voiceman, file_id, row_id, speaker, = split_label
                    full_quest_id = '_'.join((quest_label.lower(), quest_id))
                    speaker = speaker.capitalize()
                    if text:
                        data_rows.append((full_quest_id, text_id, speaker, text))
...
if __name__ == '__main__':
    ...
    ingest.process_cutscenes('raw-data/cut_scene/*/*.csv')
